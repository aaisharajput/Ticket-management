...
export default function CreateAccount(){
  const navigate=useNavigate();
    ...
    const {username,password,email,cnf,last,role}=userDetails
    ...
    function verify_field(event){
      event.preventDefault();
        if(username==""||password==""||email==""||cnf==""||role==""){
            setErrMsg("Empty field is not allowed!!");
            
          }else if(username.trim()==""||password.trim()==""||email.trim()==""||cnf.trim()=="" || last.trim()=="" || role.trim()==""){
    ...
    return(
      <>
         <SignUp msg={errMsg} disabled={disabled} last={last} value={username}  cnf={cnf} email={email} pswd={password} onChangeRole={onInputChange("role")} onChangeLast={onInputChange("last")} onChangeCnf={onInputChange("cnf")} onChangeEmail={onInputChange("email")} onChangeUser={onInputChange("username")} onChangePswd={onInputChange("password")} onClick={verify_field}/>   
      </>
    )
}
